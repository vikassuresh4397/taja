...
function renderNotification(userName){
  notificationWrapper.innerHTML=`<h5 class="notification info">hey {userName}, welcome back!</h5>`
}

function getAsTodoItem(id,title,completed){
  return `<label><input class="todo-item-checkbox" data-id="${id}" type="checkbox" ${completed?"checked":""}>${title}</input></label>`
}

function getAsTodoList(todoData){
return `<div id="todo-list-wrapper" class="todo-list-wrapper">${todoData.map((item)=>{
